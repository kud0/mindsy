-- Complete fix for usage functions and subscription plans
-- Run this in Supabase SQL Editor

-- First, ensure subscription_plans table exists with correct data
INSERT INTO public.subscription_plans (tier, name, summaries_per_month, max_file_size_mb, total_monthly_mb, output_formats, price_monthly, features) 
VALUES 
  ('free', 'Free Plan', 2, 60, 120, '["pdf"]'::jsonb, 0, '["Basic Mindsy Notes", "PDF Export", "60MB file limit"]'::jsonb),
  ('student', 'Student Plan', -1, 300, 700, '["pdf", "docx", "html"]'::jsonb, 5, '["Unlimited Mindsy Notes", "Multiple export formats", "300MB file limit", "Priority processing"]'::jsonb)
ON CONFLICT (tier) DO UPDATE SET
  name = EXCLUDED.name,
  summaries_per_month = EXCLUDED.summaries_per_month,
  max_file_size_mb = EXCLUDED.max_file_size_mb,
  total_monthly_mb = EXCLUDED.total_monthly_mb,
  output_formats = EXCLUDED.output_formats,
  price_monthly = EXCLUDED.price_monthly,
  features = EXCLUDED.features;

-- Enhanced usage limit functions for MB-based two-tier system
...
-- Function to get current usage info for UI display
