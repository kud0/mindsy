  ...
  it('should call linkGitHubAccount when link button is clicked', async () => {
    ...
    async function handleLinkAccount() {
      try {
        ...
        if (data?.url) {
          // Instead of actually navigating, just store the URL
          redirectUrl = data.url;
          // window.location.href = data.url; // This doesn't work in JSDOM
        }
    ...
    expect(redirectUrl).toBe('https://github.com/oauth');
  });
