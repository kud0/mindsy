  ...
  const loadExamReview = async () => {
    try {
      ...
      if (response.ok) {
        ...
        console.log('ExamReview - Full review data:', data);
        console.log('ExamReview - Questions array:', data.questions);
        if (data.questions && data.questions.length > 0) {
          const firstQuestion = data.questions[0];
          console.log('ExamReview - First question structure:', {
            id: firstQuestion.id,
            correctAnswer: firstQuestion.correctAnswer,
            userAnswer: firstQuestion.userAnswer,
            isCorrect: firstQuestion.isCorrect,
            hasUserAnswer: firstQuestion.userAnswer !== undefined,
            userAnswerType: typeof firstQuestion.userAnswer,
            options: Object.keys(firstQuestion.options || {})
          });
        }
      ...
      console.error('ExamReview - Error loading review:', error);
      toast({
              ...
              <CardContent className="space-y-6">
                ...
                <div className="space-y-3">
                  {Object.entries(currentQuestion.options).map(([key, value]) => {
                    ...
                    if (currentQuestionIndex === 0) { // Only log for first question to avoid spam
                      console.log(`ExamReview - Option ${key} comparison:`, {
                        optionKey: key,
                        userAnswer: currentQuestion.userAnswer,
                        correctAnswer: currentQuestion.correctAnswer,
                        isUserAnswer,
                        isCorrectAnswer,
                        userAnswerMatch: `"${currentQuestion.userAnswer}" === "${key}" = ${currentQuestion.userAnswer === key}`,
                        correctAnswerMatch: `"${currentQuestion.correctAnswer}" === "${key}" = ${currentQuestion.correctAnswer === key}`
                      });
                    }
                    ...
                    if (isCorrectAnswer) {
                      bgColor = 'bg-green-50 border-green-300 dark:bg-green-900/20 dark:border-green-600';
                      textColor = 'text-green-800 dark:text-green-100';
                      icon = <CheckCircle className="h-4 w-4 text-green-600" />;
                    } else if (isUserAnswer && !isCorrectAnswer) {
                      bgColor = 'bg-red-50 border-red-300 dark:bg-red-900/20 dark:border-red-600';
                ...
                {/* Explanation */}
                {showExplanations && (
                  <Card className="bg-blue-50 border-blue-200 dark:bg-blue-900/20 dark:border-blue-600">
